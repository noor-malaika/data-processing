...
import h5py
import numpy as np
from dataset_utils import get_nested_value
...
class ReadData(ReadRawData):
    ...
    def read(self, geom_file, constr_file, fem_file, pch_file):
        try:
            ...
            # always read fem file first cz of subcases
    ...
    def _get_node_features(self, sub_id, coords, ntypes, thickness, spc, force, disp):
        try:
            for nid, ndata in self.nodal_features[sub_id].items():
                coords.append(np.array(list(map(np.float32, get_nested_value(ndata, ['data'])))))
                ...
                force.append(np.array(list(map(np.float32, get_nested_value(ndata, ['force'])))))
                disp.append(np.array(list(map(np.float32, get_nested_value(ndata, ['y'])))))
            assert all(len(lst) == len(coords) for lst in [ntypes, thickness, spc, force, disp])
        except Exception as e:
            self.logger.error(f"Error getting node features for sub_id {sub_id}: {e}")

    def create_hdf5(self, data_file: h5py.File, variant):
        try:
            var_no = data_file.create_group(f"{variant}")
            # get edge features
            ...
            for sub_id in self.nodal_features:
                ...
                self._get_node_features(sub_id, coords, ntypes, thickness, spc, force, disp)
                # node feats
