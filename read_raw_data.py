        ...
        try:
            ...
            for eid, data in self.tria.items():
                ...
                if len(nids) != 3:  # Ensure tria has exactly 3 nodes
                    self.logger.error(
                        f"Warning: TRIA {eid} has incorrect node count: {len(nids)}"
                    )

                temp_edges = list(
                    combinations(nids, 2)
                )  # All possible 2-node edges in tria
        ...
        try:
            for eid, data in self.rb2.items():
                ...
                if master_id is None or not slave_ids:  # Ensure data is valid
                    self.logger.error(
                        f"Warning: RB2 {eid} has missing master/slave data"
                    )

                for i, sl_id in enumerate(slave_ids):
                    self.edge_features[eid][i] = {
                        "edge": (master_id, sl_id),
                        "edge_type": "1",
                    }
        except Exception as e:
    ...
    def _create_rb3_edges(self):
        ...
        try:
            for eid, data in self.rb3.items():
                ...
                if master_id is None or not slave_ids:  # Ensure data is valid
                    self.logger.error(
                        f"Warning: RB3 {eid} has missing master/slave data"
                    )
        ...
        try:
            for eid, data in self.tria.items():
                ...
                for node_id in node_ids:
                    ...
                    self.node[node_id]["thickness"] = self.pshell[pid]["data"][
                        1
                    ]  # 2nd elements in data dict is thickness for pshell
        except Exception as e:
    ...
    def _add_spc_and_force(self):
        try:
            ...
            for sub_id in self.spc.keys():
                ...
                self.nodal_features[sub_id][force_node_id]["force"] = self.force[
                    sub_id
                ]["force"]
        except Exception as e:
